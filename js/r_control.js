...
Control.prototype.checkAndOverrideMap = function() {
	...
	const newGoals = parseInt(Control.prototype.failoverNull(params.get('g')));
	if (isNaN(newGoals)) {
		return false;
	}
	...
	const newX = parseInt(Control.prototype.failoverNull(params.get('x')));
	if (isNaN(newX)) {
		return false;
	}
	...
	const newY = parseInt(Control.prototype.failoverNull(params.get('y')));
	if (isNaN(newY)) {
		return false;
	}
	...
	const newDir = parseInt(Control.prototype.failoverNull(params.get('d')));
	if (isNaN(newDir)) {
		return false;
	}
 ...
 * nullだったら '' を返す
 */
Control.prototype.failoverNull = function(v) {
	return (v === null) ? '' : v;
};
